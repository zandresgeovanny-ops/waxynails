...
(function (window, document) {
  ...
  var Abanico = (function () {
    ...
    /* Con la piel pastel, un esmalte no es UN color: son DOS.
       El pastel (data-hex) es superficie —velos, fondos, la punta del
       palillo—. Su par profundo (data-profundo) es acción y texto —botones,
       bordes activos, el rótulo impreso sobre la punta—. Si el acento del
       sitio se volviera pastel, el botón primario perdería el contraste; por
       eso se escriben los dos y --acento SIEMPRE recibe el profundo.
       El contraste NUNCA se calcula en runtime: los 24 valores están
       precalculados y verificados a AA en el archivo de sistema. */
    function repintar(el) {
      var r = document.documentElement.style;
      r.setProperty('--acento',       el.getAttribute('data-profundo'));  // acción
      r.setProperty('--acento-suave', el.getAttribute('data-hex'));       // superficie
      r.setProperty('--acento-texto', el.getAttribute('data-texto'));     // sobre la acción
    }
  ...
  function conectarWhatsApp() {
    ...
    window.WhatsApp.boton({
      telefono: tel,
      mensaje: mensajeWhatsApp(Abanico.actual() || 'Malva de Cantera'),
      apareceEn: 560,
      tooltip: '¿Te ayudo a escoger?',
      etiqueta: 'Escribir a Wax & Nails por WhatsApp'
    });

    actualizarEnlacesWA(Abanico.actual() || 'Malva de Cantera');
  }
